...
def convolve(image: np.ndarray, kernel) -> np.ndarray:   # Convolve the given filter with the image
    h, w = image.shape
...
def get_gaussian_kernel(k=3, sigma=5):
    ...
    G = 1 / (2 * np.pi * sigma ** 2) * np.exp(
        -d / (2 * sigma ** 2))  # Gaussian filter with kernel size k  and standard deviation sigma
    return G
